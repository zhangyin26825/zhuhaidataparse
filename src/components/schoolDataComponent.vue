...
export default defineComponent({
  ...
  computed: {
    ...
    errorAnswerPaperMessages() {
      ...
      return filteredAnswers.flatMap(answer => answer.error_messages);
    },
    today() {
      var today = new Date();
      // 获取年、月、日信息
  ...
  <div class="contain">
    <table style="border:1px solid #000; width: 100%">
      ...
      <tr v-for="subjectStat in subjectStats">
        ...
        <td colspan="2">{{ subjectStat.subject }}</td>
        <td colspan="2">正常数量</td>
        <td colspan="2"> {{ subjectStat.correct }}</td>
        <td colspan="2">异常数量</td>
        <td colspan="2">{{ subjectStat.error }}</td>
        <td colspan="2">缺少数量</td>
        <td colspan="2">{{ subjectStat.miss }}</td>
      </tr>
